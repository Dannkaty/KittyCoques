...
class Delicious_Dynamic_Recipe_Card {
	...
	public function render( $attributes, $content ) {
		...
		// echo '<pre>attributes'; print_r( $attributes ); echo '</pre>';
		...
		$recipe_card_heading = '
			<div class="dr-title-wrap">
				...
				'<div class="dr-entry-meta">'.
					...
					<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ). 'assets/images/sprite.svg#author"></use></svg>'. __( "Author:", "delicious-recipes" ) . " " .
	...
	public static function get_detail_items( array $details ) {
		...
		if( $difficulty) {
			$svg = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ). 'assets/images/sprite.svg#difficulty"></use></svg>';
			$output .= sprintf(
		...
		foreach ( $details as $index => $detail ) {
			...
			if( ! empty( $icon ) ) {
				$icon_svg = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ). 'assets/images/sprite.svg#'.$icon.'"></use></svg>';
			}
		...
		if( $season) {
			$svg = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ). 'assets/images/sprite.svg#season"></use></svg>';
			$output .= sprintf(
				'<span class="%1$s"><span class="%2$s">%3$s %4$s:</span><b>%5$s</b></span>',
	...
	public static function get_recipe_terms( $taxonomy ) {
		...
		if ( 'recipe-course' === $taxonomy ) {
			...
			$svg       = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ).'assets/images/sprite.svg#category"></use></svg>';
		}
		elseif ( 'recipe-cuisine' === $taxonomy ) {
			...
			$svg       = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ).'assets/images/sprite.svg#cuisine"></use></svg>';
		}
		elseif ( 'recipe-cooking-method' === $taxonomy ) {
			...
			$svg       = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ).'assets/images/sprite.svg#cooking-method"></use></svg>';
		}
		elseif ( 'recipe-key' === $taxonomy ) {
			...
			$svg       = '<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ).'assets/images/sprite.svg#recipe-keys"></use></svg>';
		}
		...
		$output = sprintf(
			'<div class="%s">
	            <a class="dr-print-trigger dr-btn-link dr-btn2" href="#%s" %s>
					<svg class="icon"><use xlink:href="'.plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ). 'assets/images/sprite.svg#print"></use></svg>
	                %s
	            </a>
	        </div>',
	...
	public static function get_pinterest_button( $media, $url, $description = '', $attributes = '' ) {
		...
		$output = sprintf(
	        ...
	        esc_url( $pinitURL ),
			$atts,
			plugin_dir_url( DELICIOUS_RECIPES_PLUGIN_FILE ).'/assets/images/pinit-sm.png'
		);
